...
void print_cache_config()
{
  cout << "itlb_set " << ITLB_SET << endl
       ...
       << endl
       << "pfb_size " << (PFB_SET * PFB_WAY * BLOCK_SIZE) / 1024 << endl
       << "pfb_set " << PFB_SET << endl
       << "pfb_way " << PFB_WAY << endl
       << "pfb_rq_size " << PFB_RQ_SIZE << endl
       << "pfb_wq_size " << PFB_WQ_SIZE << endl
       << "pfb_pq_size " << PFB_PQ_SIZE << endl
       << "pfb_mshr_size " << PFB_MSHR_SIZE << endl
       << "pfb_latency " << PFB_LATENCY << endl
       << endl
       << "cxl_channel_size " << (CXL_SET * CXL_WAY * BLOCK_SIZE) / 1024 << endl
       << "cxl_channel_set " << CXL_SET << endl
       << "cxl_channel_way " << CXL_WAY << endl
       << "cxl_channel_rq_size " << CXL_RQ_SIZE << endl
       << "cxl_channel_wq_size " << CXL_WQ_SIZE << endl
...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    {
      bool ddio = false;
      if(MSHR.entry[mshr_index].active_pref) ddio = true;
      way = llc_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type, ddio);
    }
    else if (cache_type == IS_PFB)
    {
      way = pfb_find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type, false);
    }
    else
    {
      way = find_victim(fill_cpu, MSHR.entry[mshr_index].instr_id, set, block[set], MSHR.entry[mshr_index].ip, MSHR.entry[mshr_index].full_addr, MSHR.entry[mshr_index].type, false);
    }

#ifdef CXL_BYPASS
    if (cache_type == IS_CXL) // bypass CXL
    {
      // check fill level
...
#ifdef PFB_BYPASS
    if (cache_type == IS_PFB) // bypass PFB
    {
      // PFB bypass requests sent from upper caches
      if (MSHR.entry[mshr_index].fill_level < fill_level)
      {
        if (MSHR.entry[mshr_index].instruction)
        {
    ...
    if (do_fill)
    {
      ...
      {
        cpu = fill_cpu;
        MSHR.entry[mshr_index].pf_metadata = pfb_prefetcher_cache_fill(MSHR.entry[mshr_index].address << LOG2_BLOCK_SIZE, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, block[set][way].address << LOG2_BLOCK_SIZE, MSHR.entry[mshr_index].pf_metadata);
        cpu = 0;
      }
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
  {
    ...
    { // writeback miss (or RFO miss for L1D)
      ...
      else
      {
        ...
        if (cache_type == IS_LLC)
        {
          way = llc_find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type, false);
        }
        else
          way = find_victim(writeback_cpu, WQ.entry[index].instr_id, set, block[set], WQ.entry[index].ip, WQ.entry[index].full_addr, WQ.entry[index].type, false);

#if (defined CXL_BYPASS)
        if (cache_type == IS_CXL)
        {

          if (lower_level->get_occupancy(2, WQ.entry[index].address) == lower_level->get_size(2, WQ.entry[index].address))
          {
            // lower level WQ is full, cannot insert
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
    {
      ...
      if (way >= 0) // read hit
      {
        ...
        if (RQ.entry[index].type == LOAD)
        {
          ...
          {
            cpu = read_cpu;
            pfb_prefetcher_operate(block[set][way].address << LOG2_BLOCK_SIZE, RQ.entry[index].ip, 1, RQ.entry[index].type, 0);
            cpu = 0;
          }
        }
        ...
        if (cache_type == IS_LLC)
          ...
          pfb_update_replacement_state(read_cpu, set, way, block[set][way].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type, 1);
        else
      ...
      else // read miss
      {
        ...
        if (miss_handled)
        {
          ...
          if (RQ.entry[index].type == LOAD)
          {
            ...
            {
              cpu = read_cpu;
              pfb_prefetcher_operate(RQ.entry[index].address << LOG2_BLOCK_SIZE, RQ.entry[index].ip, 0, RQ.entry[index].type, 0);
              cpu = 0;
            }
          }
  ...
  for (uint32_t i = 0; i < MAX_READ; i++)
  {
    ...
    if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
    {
      ...
      if (way >= 0) // prefetch hit
      {
        ...
        {
          pfb_update_replacement_state(prefetch_cpu, set, way, block[set][way].full_addr, PQ.entry[index].ip, 0, PQ.entry[index].type, 1);
        }
        else
        {
          ...
          if (lower_level)
          {
            if (cache_type == IS_LLC)
            {
              ...
              else
              {
                ...
                lower_level->add_pq(&PQ.entry[index]); // add it to the PFB RQ
              }
#endif
            }
            else if (cache_type == IS_PFB)
...
void CACHE::broadcast_bw(uint8_t bw_level)
{
  ...
  case IS_LLC:
    ...
    break;
  case IS_PFB:
    pfb_prefetcher_broadcast_bw(bw_level);
    break;
  }
...
void CACHE::broadcast_ipc(uint8_t ipc)
{
  if (cache_type == IS_L1D)
    ...
    pfb_prefetcher_broadcast_ipc(ipc);
}
...
void CACHE::broadcast_acc(uint32_t acc_level)
{
  ...
  case IS_PFB:
    return pfb_prefetcher_broadcast_acc(acc_level);
  }
}