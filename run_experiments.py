...
def parse_args():
    ...
    parser.add_argument('--exp_tag', type=str, default='test', help='the purpose of this experiment')
    parser.add_argument('--max_threads',type=int,default='64')
    parser.add_argument('--trace_dir', type=str, default='./traces/general', help='root directory of trace')
    ...
    parser.add_argument('--l2_pref', type=list, default=['no','scooby','bop']) 
    ...
    parser.add_argument('--cxl_latency', type=int, default=80)
    parser.add_argument('--pfb_latency', type=int, default=20)

    parser.add_argument('--pfb_sets', type=int, default=2048, help='pfb_sets = 1 to simulate CXL without prefetch buffer')
    ...
    parser.add_argument('--dram_io', type=int, default=2400)

    parser.add_argument('--branch_predictor', type=str, default="perceptron")
    parser.add_argument('--warmup_inst',type=int, default=30000000, help="gapbs 150m, general 30m")
...
def prepare_sim_cmds(args):
    ...
    for prefs in pref_combinations:
        ...
        l2_pref = prefs[1]
        pref_config_path = "./config/{}.ini".format(l2_pref)
        ...
        if args.num_cores == 1:
            for trace in os.listdir(args.trace_dir):
                ...
                if l2_pref == "no":
                    sim_cmd = "{binary} --dram_io_freq={dram_io_freq} --warmup_instructions={warmup_inst} --simulation_instructions={sim_inst} --config={pref_config} -traces {trace_dir} > {results_dir}".format(\
                    binary = os.path.join(config_result_dir, "champsim"), dram_io_freq=args.dram_io, warmup_inst = args.warmup_inst, sim_inst = args.sim_inst,  pref_config=pref_config_path, trace_dir = trace_path, results_dir = sim_results_path)
                else:
                    sim_cmd = "{binary} --warmup_instructions={warmup_inst} --simulation_instructions={sim_inst} --l2c_prefetcher_types={l2pref} --config={pref_config} -traces {trace_dir} > {results_dir}".format(\
                    binary = os.path.join(config_result_dir, "champsim"), warmup_inst = args.warmup_inst, sim_inst = args.sim_inst, l2pref = l2_pref,  pref_config=pref_config_path, trace_dir = trace_path, results_dir = sim_results_path)
